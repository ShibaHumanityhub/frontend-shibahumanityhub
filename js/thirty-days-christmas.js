...
(function () {
  ...
  function styles() {
    ...
    s.textContent = [
      ...
      '.tdx-rail{position:sticky;top:3.6rem;z-index:30;display:none;gap:.35rem;padding:.45rem .75rem;overflow-x:auto;scrollbar-width:none;background:rgba(10,6,8,.88);backdrop-filter:blur(14px);border-bottom:1px solid rgba(232,197,71,.2)}',
      '.tdx-rail::-webkit-scrollbar{display:none}',
      '@media(min-width:768px){.tdx-rail{display:flex;justify-content:center;flex-wrap:wrap}}',
      '.tdx-rail a{flex:0 0 auto;font-size:.62rem;letter-spacing:.12em;text-transform:uppercase;text-decoration:none;color:rgba(255,244,224,.55);padding:.4rem .7rem;border-radius:999px;border:1px solid transparent;transition:color .15s,border-color .15s,background .15s}',
  ...
  function buildDaysHtml(today) {
    return DAY_THEMES.map(function (d) {
      ...
      var cls = 'tdx-day' + (open || today === 0 ? ' is-open' : ' is-locked') + (isToday && today > 0 ? ' is-today' : '');
      if (today === 0 && d.day === 1) cls = 'tdx-day is-open is-today';
      return (
  ...
  function freightHtml() {
    return FREIGHTS.map(function (f) {
      ...
      return (
        '<article class="tdx-fr" data-freight="' + esc(f.id) + '">' +
          '<div class="tdx-fr-top">' +
            '<span class="tdx-fr-id">' + esc(f.id) + '</span>' +
            '<span class="tdx-fr-status">' + esc(f.status) + '</span>' +
          '</div>' +
          '<div class="tdx-fr-route">' + esc(f.route) + '</div>' +
          '<p style="font-size:.78rem;color:rgba(255,244,224,.65);margin:0 0 .25rem">' + esc(f.note) + ' · ' + f.packs + ' packs (design count)</p>' +
          '<div class="tdx-bar" aria-hidden="true"><i style="width:' + f.progress + '%"></i></div>' +
      ...
      '<div class="tdx" id="tdx-root">' +
        ...
        '<section class="tdx-sec" id="tdx-heart">' +
          ...
          '<div class="tdx-hero-band">' +
            ...
            '</div>' +
            '<p class="tdx-honest">Designed for peak season. Live cams, GPS, and prizes activate with partners, legal rules, and charity rails. Until then you can feel the full machine and stand ready.</p>' +
        ...
        '<section class="tdx-sec" id="tdx-days">' +
          ...
          '<p class="tdx-lede">' +
            ...
            'You move down the road of thirty days. Past, present, and locked future. ' +
            'Tap a day. Feel what that night is for.' +
          '</p>' +
          '<div class="tdx-detail" id="tdx-day-detail" role="region" aria-live="polite"></div>' +
          '<div class="tdx-path" id="tdx-path">' + buildDaysHtml(today) + '</div>' +
  ...
  function wire(root, today) {
    ...
    root.querySelectorAll('[data-day-btn]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var n = parseInt(btn.getAttribute('data-day-btn'), 10);
    ...
    if (form) {
      form.addEventListener('submit', function (e) {
        ...
        form.reset();
        if (document.getElementById('tdx-public')) document.getElementById('tdx-public').checked = true;
      });
    }
