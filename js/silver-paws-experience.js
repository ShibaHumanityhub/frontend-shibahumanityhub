 ...
 * Truth: facilities, reels, treasury credits are demos until partners and funding are real.
 */
(function () {
 'use strict';
 ...
 '.spx-rail{position:sticky;top:0;z-index:55;display:flex;gap:.3rem;padding:.45rem .75rem;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch;background:rgba(8,10,15,.98);border-bottom:1px solid var(--sp-line);box-shadow:0 1px 0 rgba(255,255,255,.04),0 12px 40px -20px rgba(0,0,0,.65);justify-content:flex-start;transform:translateZ(0)}',
 '@media(min-width:768px){.spx-rail{background:rgba(8,10,15,.96);backdrop-filter:blur(16px)}}',
 ...
 '.spx-mtop{display:none;position:sticky;top:0;z-index:70;align-items:center;justify-content:space-between;gap:.5rem;padding:.5rem .7rem;padding-top:max(.45rem,env(safe-area-inset-top));background:rgba(7,11,20,.99);border-bottom:1px solid var(--sp-line)}',
 '@media(min-width:768px){.spx-mtop{backdrop-filter:blur(16px);background:rgba(7,11,20,.97)}}',
 ...
 '.spx-mtabs{display:none;position:fixed;left:0;right:0;bottom:0;z-index:75;grid-template-columns:repeat(5,1fr);padding:.28rem .15rem calc(.28rem + env(safe-area-inset-bottom));background:rgba(6,8,12,.99);border-top:1px solid var(--sp-line);box-shadow:0 -16px 40px rgba(0,0,0,.55),0 -1px 0 rgba(255,255,255,.04)}',
 ...
 'body.spx-mobile #package-preview{min-height:44px!important;font-size:.8rem!important;position:sticky;bottom:calc(4.8rem + env(safe-area-inset-bottom));z-index:20;border:1px solid rgba(200,205,214,.25);backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(8,10,15,.98)!important}',
 'body.is-scrolling .spx-scan,body.is-scrolling .spx-bar > i{animation-play-state:paused!important}',
 ...
 '</div>' +
 '<p class="spx-honest">Demo lanes. No live treasury credit. No fake GPS. Click a chapter below to open it. No need to scroll the whole page.</p>' +
 '</div>' +
 '</div>' +
 '</div>' +
 '</div>'
 ...
 tag.textContent = 'Demo story · design';
